...
use StubTests\Sources\Validator\AbstractConstantFlagCheck;
...
class ClassConstantsVisibilityCheck extends AbstractConstantFlagCheck
{
    ...
    protected function describeMismatch(
        ...
        if ($reflConstant->visibility === $stubConstant->visibility) {
            return null;
        }

        return "Constant {$constantEntityId} has visibility '{$reflConstant->visibility}' in reflection "
            . "but '{$stubConstant->visibility}' in stubs";
    }
}
