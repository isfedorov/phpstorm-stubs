...
class AllStubsParserTest extends BaseTestCase
{
    ...
    public function testItDeduplicatesClassesFromMultipleDirectories()
    {
        ...
        foreach ($coreClasses as $class) {
            self::assertNotNull($class->getSourcePath(), 'Class should have a source path');
            self::assertIsArray($class->getDuplicates(), 'Class should have duplicates array');
            // Each class appears in both v1 and v2, so should have 1 duplicate
            self::assertCount(1, $class->getDuplicates(), 'Class should have 1 duplicate source path');
        }
    }
    ...
    public function testItDeduplicatesFunctionsFromMultipleDirectories()
    {
        ...
        foreach ($coreFunctions as $function) {
            self::assertNotNull($function->getSourcePath(), 'Function should have a source path');
            self::assertIsArray($function->getDuplicates(), 'Function should have duplicates array');
            // Each function appears in both v1 and v2, so should have 1 duplicate
            self::assertCount(1, $function->getDuplicates(), 'Function should have 1 duplicate source path');
        }
    }
    ...
    public function testItKeepsBothVersionsWithCrossReferences()
    {
        ...
        self::assertCount(1, $v1Database->getDuplicates(), 'v1 should have 1 duplicate reference');
        self::assertCount(1, $v2Database->getDuplicates(), 'v2 should have 1 duplicate reference');

        // Verify the cross-references point to each other
        self::assertStringContainsString('v2', $v1Database->getDuplicates()[0], 'v1 should reference v2');
        self::assertStringContainsString('v1', $v2Database->getDuplicates()[0], 'v2 should reference v1');
    }
}
