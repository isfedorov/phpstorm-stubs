...
class FunctionPhpDocConformsSignatureCheckTest extends CheckTestCase
{
    ...
    public function testIncompatibleParamTypeFails(): void
    {
        ...
        $param->setTypeFromPhpDoc('int');  // incompatible
    ...
    public function testCompatibleParamTypePasses(): void
    {
        $param = new PHPParameter('x');
        $param->setType($this->createType('string'));
        $param->setTypeFromPhpDoc('string');  // compatible
    ...
    private function makeFunctionWithPhpDoc(
        ...
        $func = $this->getMockBuilder(PHPFunction::class)
            ...
            ->onlyMethods(['getId', 'getName', 'getParameters', 'getReturnTypeFromSignature', 'getReturnTypeFromPhpDoc'])
            ->getMock();

        $func->method('getId')->willReturn($name);
        $func->method('getName')->willReturn($name);
        $func->method('getParameters')->willReturn($params);

        if ($sigReturnType !== null) {
            $func->method('getReturnTypeFromSignature')->willReturn($this->createType($sigReturnType));
        } else {
            $func->method('getReturnTypeFromSignature')->willReturn(null);
        }

        $func->method('getReturnTypeFromPhpDoc')->willReturn($docReturnType);
