...
class ConstantExistsCheckTest extends CheckTestCase
{
    ...
    private function makeConstant(string $id): PHPConstant
    {
    ...
    public function testConstantFoundInStubsPasses(): void
    {
        ...
        $stubs->method('getConstants')->willReturn([$this->makeConstant($constantId)]);
    ...
    public function testCorrectConstantAmongMultiplePasses(): void
    {
        ...
        $stubs      = $this->createMockStorageManager();
        $stubs->method('getConstants')->willReturn([
            $this->makeConstant('\\PHP_MINOR_VERSION'),
            $this->makeConstant($constantId),
            $this->makeConstant('\\PHP_RELEASE_VERSION'),
        ]);
    ...
    public function testKnownProblemSkipsCheck(): void
    {
        ...
        $result = (new ConstantExistsCheck($registry))->run($stubs, $constantId, PhpVersions::LATEST->value);
    ...
    public function testKnownProblemOnlySkipsForVersionRange(): void
    {
        ...
        $result = (new ConstantExistsCheck($registry))->run($stubs, $constantId, PhpVersions::PHP_8_1->value);
        $this->assertFalse($result->hasFailures());

        // Outside the version range (8.4) — NOT skipped, still missing → fails
        $result = (new ConstantExistsCheck($registry))->run($stubs, $constantId, PhpVersions::PHP_8_4->value);
        $this->assertTrue($result->hasFailures());
    }
}
